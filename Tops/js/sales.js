...
function setPayment() {
	...
	var total = document.getElementById('hidden_total').value;
	if( (Number(payment_value) >= Number(total)) && (total != 0)) {
		btn_payment.removeAttribute('disabled');
		hidden = true;
	} else {
		btn_payment.setAttribute("disabled", "disabled");
	}
}
function invoiceSet() {
	var total = document.getElementById('hidden_total').value; 
	var zero = 0;
	var invoice = document.getElementById('inv_no').value;
	var total = document.getElementById('total_total').value;
	var inv_method = 'Cash';
	var payment_value = document.getElementById('payment_value').value;
	if(hidden != true) {
		inv_method = 'Credit Card';
	}
	var http = new XMLHttpRequest();
	http.onreadystatechange = function() {
		if(this.readyState == 4 && this.status == 200) {
	...
	http.open("POST",''+myurl+'/tops/include/php/pos/createInvoice',true);
	http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	http.send("transaction&total="+total+"&invoice_method="+inv_method);
}
function updateInventory() {
	...
	http.open("GET",''+myurl+'/tops/include/php/update_stock.php',true);
	http.send();
}
function invoiceOut() {
	var http = new XMLHttpRequest();
...
function paymentDone() {
	...
	zero = payment_value - total;
	document.getElementById('change').innerHTML ="P"+ zero.toLocaleString(undefined,  { minimumFractionDigits: 2 }  );
	var http = new XMLHttpRequest();
	http.onreadystatechange = function() {
		if(this.readyState == 4 && this.status == 200) {
			fetchCart();
	...
	http.open("POST",''+myurl+'/tops/include/php/pos/createTransaction',true);
	http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	http.send("transaction&total="+zero+"&inv_no="+invoice+"&invoice_method="+inv_method);
}
function printReciept(e) {
